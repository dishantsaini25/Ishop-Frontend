...
export default function EditCategory({ category, imageBaseUrl }) {
  ...
  const [previewImage, setPreviewImage] = useState(imageBaseUrl + category.image);
  ...
  function submitHandler(event) {
    ...
    axiosInstance.put(`category/update/${category._id}`, form)
      .then((response) => {
                ...
                {previewImage && (
                  <div className="flex items-center gap-3 sm:gap-4 p-3 sm:p-4 bg-gray-50 rounded-xl">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-400 break-all">{category.image}</p>
                    </div>
                    {previewImage !== (imageBaseUrl + category.image) && (
                      <button
                        type="button"
                        onClick={() => setPreviewImage(imageBaseUrl + category.image)}
