...
const MOBILE_SLUGS = ["mobile", "mobiles", "smartphone", "smartphones", "phone", "phones", "cellphone"];
const PER_PAGE = 5;
...
export default function TopCellphones() {
  ...
  const [categorySlug, setCategorySlug] = useState("mobile");
  ...
  // Try each mobile slug until we find products
  const findAndFetch = async () => {
    ...
    // No mobile products found - show all products
  ...
  const imgSrc = (t) => t?.startsWith('http') ? t : (t ? `${imageBaseUrl}main/${t}` : "/image/Top-phone.png");

  return (
    <section className="w-full rounded-3xl bg-white border border-[#E5E5E5] shadow-sm">
      <div className="flex items-center justify-between px-6 pt-5">
        <h2 className="text-[14px] font-semibold uppercase tracking-wide text-[#111827]">TOP CELLPHONES &amp; TABLETS</h2>
      ...
      <div className="px-6 pt-4">
        <div className="relative h-40 overflow-hidden rounded-[20px] bg-[#111827] group cursor-pointer" onClick={() => router.push(`/store?category=${categorySlug}&page=1`)}>
          <Image src="/image/Redmi.png" alt="Mobiles" fill className="object-cover transition-transform duration-500 group-hover:scale-105" />
          <div className="relative z-10 flex h-full flex-col justify-center px-6 text-white">
            <h3 className="text-[16px] font-semibold">TOP MOBILES<br />&amp; TABLETS</h3>
            <button className="mt-3 w-max rounded-full bg-black px-4 py-2 text-[11px] font-semibold text-white hover:bg-black/80 transition">SHOP NOW</button>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-5 px-4 py-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
        {loading ? [...Array(PER_PAGE)].map((_, i) => (
          ...
          <div className="col-span-full text-center py-8 text-gray-500">No mobile products found</div>
          ...
          <article key={product._id} onClick={() => router.push(`/product/${product.slug}`)}
            ...
            <div className="relative flex items-center justify-center rounded-[20px] bg-white py-4">
              ...
              <img src={imgSrc(product.thumbnail)} alt={product.name} className="h-40 w-auto object-contain group-hover:scale-105 transition duration-300" onError={(e) => { e.target.src = "/image/Top-phone.png"; }} />
            </div>
